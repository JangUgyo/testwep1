        function setupRealtime() {
            ...
            STATE._rt = sb.channel('portal-realtime')
                ...
                .on('postgres_changes', { event: '*', schema: 'public', table: 'trade_documents' }, async () => { await reloadTrade(); renderDashboardWidgets(); if (STATE.currentTab === 'trade') renderTrade(); if (STATE.currentTab === 'inventory' && STATE.inventoryView === 'receiving') renderReceivingStatus(); refreshOpenDetail('trade'); })
                .on('postgres_changes', { event: '*', schema: 'public', table: 'po_receipts' }, async () => { await reloadPoReceipts(); if (STATE.currentTab === 'inventory' && STATE.inventoryView === 'receiving') renderReceivingStatus(); })
